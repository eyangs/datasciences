...
## Data Description
The data is from 6 participants who wore accelerometers on the belt, forearm, arm, and dumbbells during exercise routines. The data are split into a training group (19,622) observations and testing group (20 observations). 
...
```r
training_data <- read.csv(file.path("~/R/coursera/github/datascience/pml", "pml-training.csv"), na.strings = c("NA", "#DIV/0!", ""))
test_data <- read.csv(file.path("~/R/coursera/github/datascience/pml", "pml-testing.csv"), na.strings = c("NA", "#DIV/0!", ""))
```
...
### Exploratory Analysis, Plot the frequencies
...
![plot of chunk unnamed-chunk-6](rplot.png) 
